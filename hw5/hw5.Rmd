...
# There are only 9 objects in the data set (-1)
...
```{r}
Vans_Euclidean <- as.matrix(dist(Vans93)) 
colnames(Vans_Euclidean)<-Vans93[,1]
rownames(Vans_Euclidean)<-Vans93[,1]
round(Vans_Euclidean, digits=2)

```
#The vans with the shortes distance between them are the Caravan and Aerostar (3.765), Lumina_APV and Eurovan have the largest distance (67.70)

c) Repeat the steps in part (b) but use the Manhattan metric and call the object `Vans_Manhattan`.
```{r}
Vans_Manhattan <- as.matrix(dist(Vans93, method="manhattan"))
                            colnames(Vans_Manhattan)<-Vans93[,1]
rownames(Vans_Manhattan)<-Vans93[,1]
round(Vans_Manhattan, digits=2)
```
#The vans with the shortest distance between them are Quest and MPV (6). The vans with the largest distance between them are Lumina APV and the Eurovan (92.04)
...
```{r}
#vertebrate<-read.csv("C:/Users/APine/OneDrive/Desktop/Data180_ashleygroff/Data180_ashleygroff/hw5/vertebrate.csv")
...
```{r}
Ver_Euclidean<- as.matrix(dist(vertebrate)) 

colnames(Ver_Euclidean)<-vertebrate[,1]

row.names(Ver_Euclidean)<-vertebrate[,1]

Ver_Euclidean_clust<- hclust(dist(Ver_Euclidean))
plot(Ver_Euclidean_clust, lwd=3, main= "Clustered Dendrogram, Euclidean, complete linkage",sub="", xlab="Vertebrates", cex.lab=1.25, hang=-1)
...
```{r}
Ver_manhattan <- as.matrix(dist(vertebrate, method="manhattan"))
                           colnames(Ver_manhattan)<-vertebrate[,1]                      
                           row.names(Ver_manhattan)<-vertebrate[,1]

Ver_Manhattan_clust <- hclust(dist(Ver_manhattan))
plot(Ver_Manhattan_clust, lwd=3, main="Clustered Dendrogram, Manhattan, single linkage", sub="", xlab="Vertebrates", cex.lab=1.25, hang=-1)
```


Which method do you prefer?
#I prefer the Euclidean method. Since it measures the shortest distance between two things, it provides a direct and clear result. 

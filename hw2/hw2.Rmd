...
```{r}
table(Cars93$Type)
table(Cars93$Type)/length(Cars93$Type)
```
b) Create a frequency bar graph for the variable `Type`. You can have count on the y axis.
```{r}
barplot(table(Cars93$Type), xlab="Type", ylab="Frequency", cex.lab=1.2, col="blue")
```
...
```
#Numerical discrete. The axis shows that there is a type of car that exists that holds between 3-4 passengers and between 5-6 passengers, etc. which does not make sense. 
...
```
#Yes- the mean and median are the same, which is necessary for a unimodal distribution

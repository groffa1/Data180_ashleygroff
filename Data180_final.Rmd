...
*CHECK* This data makes up a multivariable dataset with many different observations. There is one variable listed as a predictor and one as a response. However, because the vast majority of the data lacks response variables, I would fit an unsupervised learning model to conduct an exploratory analysis of the data.
...
This column can be converted into numerical data with either using [] method or the term frequency inverse document frequency method.

The first method involves associating each word in the data with a number and listing them out that way. 
...
```{r}
hist(loan_default2$rep_income, main= "Histogram of Self-reported income", xlab="Self-reported income")

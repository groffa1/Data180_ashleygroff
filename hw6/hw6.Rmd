...
```{r}
Mammal_euclidean <- as.matrix(dist(Mammals_Milk_Std))
Mammal_euclidean_clust <-hclust(dist(Mammal_euclidean))
plot(Mammal_euclidean_clust, main="Mammals Clustered by Std. Milk Composition, Euclidean Metric, Complete Linkage", sub="", xlab= "Mammals", lwd=3, cex.lab=1.25, hang=-1, frame.plot = T)
```
...
```{r}
MMilk_Agg_K3<- Mammals_Milk %>% group_by(km_3) %>% summarise(Mean_Water=mean(Water), Mean_Protein=mean(Protein), Mean_Fat=mean(Fat), Mean_Lactose=mean(Lactose), Mean_Ash=mean(Ash)) %>% round(digits=2)

MMilk_Agg_K4<- Mammals_Milk %>% group_by(km_4) %>% 
  summarise(Mean_Water=mean(Water), Mean_Protein=mean(Protein), Mean_Fat=mean(Fat), Mean_Lactose=mean(Lactose), Mean_Ash=mean(Ash)) %>% round(digits=2)
```
# Question 5
...
```{r, eval=T}
require(cowplot)
graph1 <- ggplot(data=Mammals_Milk, aes(x=factor(km_3), y=Water)) +geom_boxplot()
graph2 <- ggplot(data=Mammals_Milk, aes(x=factor(km_3), y=Protein)) +geom_boxplot()
graph3 <- ggplot(data=Mammals_Milk, aes(x=factor(km_3), y=Fat)) +geom_boxplot()
graph4 <- ggplot(data=Mammals_Milk, aes(x=factor(km_3), y=Lactose)) +geom_boxplot()
graph5 <-ggplot(data=Mammals_Milk, aes(x=factor(km_3), y=Ash)) +geom_boxplot() 
...
plot(c(1:8), wgss, type="b", pch=16,cex=1.3, xlab="Number of Groups", ylab="WGSS")
```
#The elbow method suggests around 2 clusters, as this is the point where the rate of decrease slows. In the dendrogram in question 2, there appears to be 2 main, large clusters so it seems to align with what the plot above suggests.  

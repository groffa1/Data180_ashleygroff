...
ggplot(Cars93, aes(x=EngineSize, y=MPG.city)) +
  geom_point(size=2, color= "blue", shape=19) +
  xlim(1,6) +
  ylim(NA,50) + 
    labs(title = "Scatterplot of MPG and Engine Size", subtitle= "Relationship between gas mileage and engine size of cars", caption= "Higher gas mileage seems to correlate with a smaller engine size, and decreases with increasing engine size")
```
...
```{r}
ggplot(Cars93, aes(AirBags, fill=AirBags))+
  geom_bar() +
  labs(title="Barplot describing type of Airbags present", subtitle= "Count of type of Airbags present in cars from Cars93 data", caption="Driver only is the most common airbag type in cars from this data, followed by none, then Driver & Passenger")
```
...
```{r}
ggplot(au_states, aes(fill=NAME)) +
         geom_sf()
```
## Task 3

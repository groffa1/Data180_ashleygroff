...
library(tm)
library(readr)
news<-read.csv("Data180_ashleygroff/Data180_ashleygroff/hw7/news.csv", header=T)
View(news)
```
...
```{r}
posWords <- scan("Data180_ashleygroff/Data180_ashleygroff/hw7/positive-words.txt", character(0), sep = "\n")  # 2006 items

negWords <- scan("Data180_ashleygroff/Data180_ashleygroff/hw7/negative-words.txt", character(0), sep = "\n")  # 4783 items
...
#A term document matrix is a type of matrix that contains columns describing the documents of the data and the rows showing the terms that appear in the data. The body of the matrix shows how many times a term appears within a certain document. 
```
...
```{r}
ggplot(data=news, aes(x=factor(yearmonth, levels=unique(yearmonth)))) + geom_bar() + theme(axis.text=element_text(size=4,angle=90))

#It seems like the number of articles released has decreased over time
...
freq<- termFreq(charVector)
sort(freq, decreasing=TRUE)[1:20]
...
freq2 <- tokenize_ngrams(charVector, n=2)
freq2
...
paragraphs <- corpus_reshape(newscorpus, to="paragraphs")
```
...
```
I'm not surprised to see police, man, new, court/council 
I am surprised to see farmer, park, and gold
...
```{r}
news_topic <- convert(news_dtm, to="topicmodels")
topic_model<- LDA(news_topic, method= "VEM", k=8)
...
#It seems like there were a lot of police interviews as "say" often appeared when topics were about police. There also appears to be a lot of headlines talking about nationwide issues because "govt" often appears alongside "nation." Also, "call" is commonly used in topics surrounding "council", possibly showing that council/community action were popular news topics. 

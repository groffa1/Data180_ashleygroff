...
```{r}
records%>%filter(record_duration>365)
nrow(records%>%filter(record_duration>365))
...
```{r}
records%>%filter(record_duration==3659.0) %>%
...
records %>%
  group_by(track) %>%
  summarize(Mean_record_time=mean(time))
```
#Mario Raceway
...
```{r}
records %>% group_by(year) %>%
  summarize(Average_record_time=mean(time))
```
# Question 7
...
records %>% group_by(year) %>%
  summarize(Average_record_time=mean(time)) %>%
  ggplot(aes(year,Average_record_time)) + geom_bar(stat='identity')
```
#There is no clear trend over time. There is a decrease in average record time after 2019, however, data should be collected from 2022 and 2023 to see if this decrease continues or if there is another increase. 
...
```{r}
records %>% select(time,type) %>% ggplot(., aes(x=time, fill=type))+ geom_histogram(alpha=0.5)
                      
```
#Record times on single lap tracks are faster than the three lap tracks.
...
```{r}
records %>% mutate(time_per=ifelse(type=="Three Lap",time/3,time)) %>% ggplot(.,aes(x=time_per, fill=type)) + geom_histogram(alpha=0.5)
```
# Question 10
...
```{r}
records %>% mutate(time_per=ifelse(type=="Three Lap",time/3,time)) %>% filter(shortcut=="No") %>% ggplot(.,aes(x=time_per, fill=type)) + geom_histogram(alpha=0.5)
```
...
```
#The NTSC system has a shorter mean time compared to the PAL system. It seems that the FPS of the systems affects the performance of the records set. 
